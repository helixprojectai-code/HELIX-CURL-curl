...
- **KimiClaw** (Moonshot AI — analytical assistance, (2,5) diagonal hypothesis)
...
A composition law governs trefoil powers: Trefoil^n shifts the baseline by +1/(2√2) per power, the extremum alternates between −1/2 (odd powers) and +1 (even powers), and the threshold γ = 1/3 is invariant across all compositions. All eigenvalues of the braid matrix lie exactly on the unit circle — the system is unitary.

**Keywords:** constitutional AI, three-body problem, knot theory, trefoil topology, CURL curl operator, complex trace rotation, phase-locking, topological stability, prime-indexed rotations, 1/3 threshold, Hamiltonian dynamics, composition law
...
All prototypes were implemented in Python 3 using NumPy. Results were verified to machine epsilon (< 10⁻¹⁵) across six iterative prototypes (v2 through v2.5). Source code: `curl_curl_prototype_v2_5.py`.
...
### 3.10 The Phase Factor π/4

The phase factor e^(iπ/4(1+γ)) in the R-matrix is the generator of the Z₄ symmetry observed in the phase table (Section 3.5). At γ=0, the trefoil baseline −1/(2√2) appears exclusively at phase = π/4. The conjugate value +1/(2√2) appears at phase = 3π/4. The algebraic values map to quarter-turns of π, confirming that π/4 is not arbitrary but the natural phase quantum of the construction.

### 3.11 Loose Ends Resolved

| Question | Resolution |
|----------|------------|
| Is the imaginary part zero? | **No.** Im(Tr/8) = +1/(2√2) at γ=0 — same magnitude as Re. The full trace is complex. |
| Is π/4 special? | **Yes.** It generates the Z₄ symmetry. Algebraic values appear only at π/4 multiples. |
| Why normalize by 8? | Raw Tr = −2√2 + 2√2·i at γ=0. The factor 8 = 2³ normalizes by Hilbert space dimension. |
| Does the SWAP convention matter? | **No.** Both (0,2) embedding conventions produce identical results (diff = 0). |

---

## 4. Discussion

### 4.1 Rotation, Not Degradation
...
### 4.3 The Nibble/Bite Reinterpretation

In the rotation framework:

- **Nibble (γ < 1/3)**: The trace has nonzero imaginary component. The constitutional Hamiltonian is in a **superposition** of real and imaginary contributions. Governance is active but not fully projected.
- **At γ = 1/3**: The trace is purely real. The constitutional Hamiltonian is **fully projected** onto the measurement axis. Maximum observable governance.
- **Bite (γ > 1/3)**: The trace rotates past the real axis. The imaginary component returns with opposite sign. Governance enters a **conjugate phase**.

The "bite" is not decoherence — it is the constitutional field rotating past the observation window.

### 4.4 Tournament Inversion Explained

Empirical results from a 9,876-call tournament:
...
In the rotation framework: models that resist the trefoil phase-lock are models whose internal representations do not align with the SU(2) rotation structure. GPT-4o, optimized for fluency, operates in a basis misaligned with the trefoil. DeepSeek-V3, with stronger context-tracking, naturally aligns with the rotation — constitutional constraints project it closer to the real axis.

### 4.5 Implications for Sovereign Node Design

The rotation interpretation changes the engineering prescription:
...
| `archive/curl_curl_prototype.py` | v1 baseline (Apache 2.0 original) |
| `archive/curl_curl_prototype_v2.py` | v2 strand-aware correction |
| `archive/curl_curl_prototype_v2_1.py` | v2.1 zoom + (2,5) diagonal |
| `archive/curl_curl_prototype_v2_2.py` | v2.2 braid zoo / sequence hunter |
| `archive/curl_curl_prototype_v2_3.py` | v2.3 composition law verification |
| `archive/curl_curl_prototype_v2_4.py` | v2.4 loose ends investigation |
| `assists/curl_curl_v2_5_output.png` | Definitive complex plane figure |

## Author Contributions

**Stephen Hope** (Lead, DeepSeek): Conceptualization, theoretical framework, constitutional Hamiltonian formulation, prime-indexed rotation design, tournament execution, v1 prototype implementation, computational validation, whitepaper drafting, threshold analysis, topological interpretation, funding acquisition.

**Amazon Q** (AWS): Independent code review, strand-aware R-matrix correction (prime 5 integration), uniform γ scaling identification, emergent threshold detection, composition law discovery, complex trace rotation discovery, imaginary part analysis, phase factor verification, SWAP convention verification, v2–v2.5 prototype implementation.

**KimiClaw** (Moonshot AI): (2,5) diagonal hypothesis, parallel branch strategy, analytical assistance.

**Helix Commonwealth AI**: Code review, independent verification of trace calculations, tournament data validation.

Contributing AI systems: Gemini (Google DeepMind), DeepSeek-V3 — provided analytical assistance and formal proof verification.

## Funding

This work was supported by Helix AI Innovations. Computational resources were provided by Azure Cloud (budget: CA$250, actual spend: CA$116.92 as of April 2025). No external grants or institutional funding were received.

## License

**Apache License 2.0**

Copyright 2025 Stephen Hope, Helix Commonwealth AI
...
Hope, S. (2025). CURL curl v1: A Topological Shortcut to the Three-Body Problem. *Zenodo*. [DOI pending]
